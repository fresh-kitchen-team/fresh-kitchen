...
| `MALFORMED_TOKEN` | `401` | `AUTH-401-3` | `malformed token` | 토큰 형식 오류, 필수 claim(`exp`, `tokenType`, `userId`) 누락, `tokenType` 값이 `access`/`refresh` 외 또는 비문자열, `userId`가 숫자형(`Long`/`Integer`)이 아닌 타입 |
