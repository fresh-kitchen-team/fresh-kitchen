...
| `MALFORMED_TOKEN` | `401` | `AUTH-401-3` | `malformed token` | 토큰 형식 오류, 필수 claim(`exp`, `tokenType`, `userId`) 누락, `tokenType` 값이 기대하는 타입(`access`/`refresh`)과 불일치 또는 비문자열, `userId`가 숫자형(`Long`/`Integer`)이 아닌 타입, access 토큰에서 `role` claim이 누락/빈 문자열이거나 `Role` enum에 정의된 값이 아님 |
